...
#subset the promoter sequence (50 bp)
...
MakeDatabase<-' __./GuideFinderMac/makeblastdb __ /ncbi-blast-2.6.0+/bin/makeblastdb -in __mutansexamplefastafile.txt__ -dbtype nucl -out GenomeDatabase'  
RunBlast<-'/ __./GuideFinderMac/blastn__ ncbi-blast-2.6.0+/bin/blastn -task blastn-short -query BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out MatchGuides.blast'  

```{r}
MakeDatabase<-'./GuideFinderMac/makeblastdb -in mutansexamplefastafile.txt -dbtype nucl -out GenomeDatabase'
RunBlast<-'./GuideFinderMac/blastn -task blastn-short -query BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out MatchGuides.blast'
```


##3.Input FASTA File
Add the name of the fasta file that contains the nucleotide sequence for your genome  
__Example__: Substitute the bolded information for the name of your FASTA file (kept in working directory)  
InputGenome<- readDNAStringSet( __"StrepMutansFASTA.fna"__)
```{r, tidy=TRUE}
InputGenome<- readDNAStringSet("mutansexamplefastafile.txt")
...
This loop takes the gene coordinates and modifies them to include the putative promoter region (50 bp upstream of the gene start)
...
for (i in 1:nrow(CoordinatesDF)) { 
  ...
  Temp<-if (WorkingRow$strand == "-") {replace(WorkingRow,4,Add50(WorkingRow$end))} #for genes coded on the minus strand, add 50 bp to the end coordinate; capture putative promoter sequence 
  else {replace(WorkingRow,3, Subtract50(WorkingRow$start))} #for genes coded on the plus strand, subtract 50 bp from the start coordinate; capture putative promoter 
...
Re_RunBlast<-' __/Users/spotom/Desktop__ /ncbi-blast-2.6.0+/bin/blastn -task blastn-short -query re_BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out re_MatchGuides.blast'  
Re_RunBlast_Lowered<-' __/Users/spotom/Desktop/__ ncbi-blast-2.6.0+/bin/blastn -task blastn-short -query re_BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 14 -out re_MatchGuides.blast'  
```{r,tidy=TRUE}
Re_RunBlast<-'./GuideFinderMac/blastn -task blastn-short -query re_BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out re_MatchGuides.blast'
#If selecting optio to relax off-target searches 
Re_RunBlast_Lowered<-'./GuideFinderMac/blastn -task blastn-short -query re_BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 14 -out re_MatchGuides.blast'
```
...
#takes 50 bp region upstream of gene; designated on + strand, it takes -50 from  
#start and on - strand it takes + 50 from end  
...
for (i in 1:nrow(GeneProductsDF)) { 
  ...
  Temp<-if (StrandType == "MINUS") {replace(WorkingRow,4,Add50(WorkingRow$end))} 
  else {replace(WorkingRow,3, Subtract50(WorkingRow$start))} 
  NewCoordinatesDF<-rbind(NewCoordinatesDF,Temp) }
}
...
names(FilteredGuides)<-c("ID","GeneProduct","Guide1","GC_G1","G1_AString",
                         "G1_TString","Guide2","GC_G2","G2_AString","G2_TString",
                         "Guide3","GC_G3","G3_AString","G3_TString",
...
CompleteGuidesList_Thresholds<-replace(CompleteGuidesList_Thresholds,3,
                                       RemoveNGG(CompleteGuidesList$Guide1)) 
CompleteGuidesList_Thresholds<-replace(CompleteGuidesList_Thresholds,7,
                                       RemoveNGG(CompleteGuidesList$Guide2))
CompleteGuidesList_Thresholds<-replace(CompleteGuidesList_Thresholds,11,
                                       RemoveNGG(CompleteGuidesList$Guide3))
...
FilteredGuides4<-data.frame(lapply(FilteredGuides4, 
                                   as.character), stringsAsFactors = FALSE)
FilteredGuides4$GuideID<-1:nrow(FilteredGuides4) #add GUIDE ID to each guide
```
...
ShortenedGuides<-data.frame(cbind(FilteredGuides4$ID, 
                                  FilteredGuides4$GeneProduct,
                                  FilteredGuides4$Guide1,
                                  lapply(FilteredGuides4$Guide1, ShortenGuide)),
                            stringsAsFactors = F)
...
GuidesWithoutOffTarget<-merge(OneHit, FilteredGuides4, by="GuideID")
...
names(GuidesWithoutOffTarget)<-c("IDs","ID","GeneProduct","Guide1","GC_G1",
                                 "G1_AString","G1_TString","Guide2","GC_G2",
                                 "G2_AString","G2_TString","Guide3","GC_G3",
                                 "G3_AString","G3_TString",
                                 "DistanceofGuide_AsPercentageofCDS","StartofGuideLocation")
...
CompleteFilteredGuides<-data.frame(lapply(GuidesWithoutOffTarget, 
                                          as.character), stringsAsFactors=FALSE)
CompleteFilteredGuides<-replace(CompleteFilteredGuides,3,
                                RemoveNGG(CompleteFilteredGuides$Guide1)) 
CompleteFilteredGuides<-replace(CompleteFilteredGuides,7,
                                RemoveNGG(CompleteFilteredGuides$Guide2))
CompleteFilteredGuides<-replace(CompleteFilteredGuides,11,
...
```{r, eval=FALSE}
#Using the FilteredGuides data frame, create a list of data frames
#sorted by gene
...
#cleaning up TopHits guides: removing NAs from data frame and removing NGG from each guide 
...
Identifies genes that did not produce any usable guides. This list is output in a .csv document called "GenesWithoutGuides" Thesholds can be lowered and this list of genes run through again. 
```{r, eval=FALSE,tidy=TRUE}
...
GuidesWithThresholds<-cbind(GuidesWithFeatures$GeneProduct,
                            ...
                            lapply(GuidesWithFeatures$Guide3,TestingStrings),
                            lapply(GuidesWithFeatures$Guide3,TestingStrings_T),
                            GuidesWithFeatures$DistanceofFGuide_AsPercentageofCDS, 
                            GuidesWithFeatures$StartofGuide)
...
names(GuidesWithThresholds)<-c("ID","GeneProduct","Guide1","GC_G1","G1_AString",
                               "G1_TString","Guide2","GC_G2","G2_AString","G2_TString",
                               "Guide3","GC_G3","G3_AString", 
                               "G3_TString", "DistanceofGuide_AsPercentageofCDS",
                               "StartofGuideLocation")
...
Complete_Genes_ReFilter<-replace(Complete_Genes_ReFilter, 7, 
                                 RemoveNGG(as.character(Complete_Genes_ReFilter$Guide2)))
Complete_Genes_ReFilter<-replace(Complete_Genes_ReFilter, 11, 
...
TestDistance<-lapply(GenesReFilter2$DistanceofGuide_AsPercentageofCDS,L_TestingDistance)
GenesReFilter2$DistanceofGuide_AsPercentageofCDS<-TestDistance
GenesReFilter3<-subset(GenesReFilter2, 
                       DistanceofGuide_AsPercentageofCDS != "N/A", 
                       select=ID:StartofGuideLocation)

GenesReFilter3<-GenesReFilter3[!grepl("N", GenesReFilter3$Guide3),] #remove guides that contain "N"


#flatten lists 
GenesReFilter3$Guide1<-unlist(GenesReFilter3$Guide1)
GenesReFilter3$Guide2<-unlist(GenesReFilter3$Guide2)
GenesReFilter3$Guide3<-unlist(GenesReFilter3$Guide3)
GenesReFilter3<-subset(GenesReFilter3, nchar(Guide1) >= N, select=ID:StartofGuideLocation)
GenesReFilter3$GuideID<-1:nrow(GenesReFilter3)
...
ReMatchGuides<-data.frame(cbind(GenesReFilter3$ID, 
                                GenesReFilter3$GeneProduct,
                                GenesReFilter3$Guide1,
                                lapply(GenesReFilter3$Guide1, ShortenGuide)),
                                stringsAsFactors = F)
...
Re_GuidesWithoutOffTarget<-merge(Re_OneHit, GenesReFilter3, by="GuideID")
...
for (i in 1: length(Re_SplitByIds)) {
  ...
  MySplitByIdDF<-replace(MySplitByIdDF, 14, as.numeric(MySplitByIdDF$StartofGuideLocation))
