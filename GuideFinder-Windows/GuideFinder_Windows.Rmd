...
```{r setup}
knitr::opts_knit$set(root.dir = '\\Users\\mspoto\\Desktop\\')
setwd('\\Users\\mspoto\\Desktop\\')
...
Default is set to identify the blastn file held in the Guide.FinderWINDOWS file.
__Example__: Substitute bolded information in MakeDatabase and RunBlast  
MakeDatabase<-' .\\Guide.Finder.WINDOWS\\makeblastdb.exe -in __mutansexamplefastafile.txt__ -dbtype nucl -out GenomeDatabase'  
RunBlast<-'.\\Guide.Finder.WINDOWS\\blastn.exe ncbi-blast-2.6.0+/bin/blastn -task blastn-short -query BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out MatchGuides.blast'  

```{r}
MakeDatabase<-'.\\Guide.Finder.WINDOWS\\makeblastdb.exe -in mutansexamplefastafile.txt -dbtype nucl -out GenomeDatabase'
RunBlast<-'.\\Guide.Finder.WINDOWS\\blastn.exe -task blastn-short -query BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out MatchGuides.blast'
```
...
Default is set to identify the blastn file held in the Guide.FinderWINDOWS file.
```{r,tidy=TRUE}
Re_RunBlast<-'.\\Guide.Finder.WINDOWS\\blastn.exe -task blastn-short -query re_BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 13 -out re_MatchGuides.blast'
#If selecting optio to relax off-target searches 
Re_RunBlast_Lowered<-'.\\Guide.Finder.WINDOWS\\blastn.exe -task blastn-short -query re_BLASTguides.fasta -db GenomeDatabase -outfmt 6 -num_threads 4 -dust no -soft_masking false -max_target_seqs 5 -word_size 14 -out re_MatchGuides.blast'
```
